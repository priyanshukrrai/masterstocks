...
export function useMarkets() {
  ...
  // Real-time crypto via Binance WebSocket (push-based, ~1s) ------------------
  // Polling always feels laggy: each update waits out an interval AND a request
  // round-trip. A WebSocket pushes prices the moment they change, so crypto
  // updates with near-zero lag. We use the all-market mini-ticker stream (one
  // connection that auto-covers every displayed coin), buffer incoming ticks
  // and flush them to state once a second to stay efficient. The REST poller
  // above stays as a fallback for networks that block WebSockets. Set
  // NEXT_PUBLIC_DISABLE_WS=true to turn this off.
  useEffect(() => {
    if (process.env.NEXT_PUBLIC_DISABLE_WS === "true") return;
    ...
    const flush = () => {
      ...
      applyProxyQuotes(setCrypto, snapshot);
    };
    ...
    function connect() {
      ...
      ws.onmessage = (ev) => {
        try {
          ...
          for (const t of arr) {
            const s = t.s || "";
            if (s.length <= 4 || s.slice(-4) !== "USDT") continue;
