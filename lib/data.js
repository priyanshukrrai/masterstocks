...
// ---- Real-time crypto (Binance public REST: key-free, CORS-enabled) --------
// CoinGecko's free /coins/markets endpoint only refreshes every ~30-60s, so a
// crypto price held steady between polls looks frozen. Binance's public ticker
// updates every second, so we overlay it on top of the CoinGecko list to make
// prices move in real time. Stablecoins and any coin without a Binance USDT
// spot pair are skipped (they keep their CoinGecko value).
...
// Cache the set of base assets that trade as a USDT spot pair on Binance, so we
// never send an unknown symbol (Binance rejects an entire batch if one symbol
// is invalid). Fetched once per page load.
let _binanceBases = null;
let _binanceBasesPromise = null;

export async function binanceUsdtBases() {
  if (_binanceBases) return _binanceBases;
  if (_binanceBasesPromise) return _binanceBasesPromise;
  _binanceBasesPromise = (async () => {
    try {
      const r = await fetch(
        BINANCE_BASE + "/api/v3/exchangeInfo?permissions=SPOT",
      );
  ...
  return _binanceBasesPromise;
}

// Map the given crypto assets to their Binance USDT pair symbols, keeping only
// pairs Binance actually lists. Returns { "BTCUSDT": "BTC", ... }.
async function binancePairMap(assets) {
  const bases = await binanceUsdtBases();
  if (!bases.size) return {}; // exchangeInfo unavailable: skip the overlay
  const map = {};
  for (const a of assets) {
    if (!a || a.kind !== "crypto") continue;
    const base = (a.symbol || "").toUpperCase();
    if (!base || STABLES.has(base) || !bases.has(base)) continue;
    map[base + "USDT"] = a.symbol;
  }
  return map;
}

// Fetch real-time prices for the given crypto assets from Binance. Returns a
// map keyed by the app symbol: { price, chg }. Empty if Binance is unreachable.
export async function fetchBinanceQuotes(assets) {
  const pairMap = await binancePairMap(assets);
  const pairs = Object.keys(pairMap);
  if (!pairs.length) return {};
  const out = {};
  const CHUNK = 100; // keep each request URL comfortably short
  for (let i = 0; i < pairs.length; i += CHUNK) {
    const group = pairs.slice(i, i + CHUNK);
    try {
      const url =
        BINANCE_BASE +
      ...
      for (const t of arr) {
        const app = pairMap[t.symbol];
        if (!app) continue;
        const price = Number(t.lastPrice);
        if (!isFinite(price) || price === 0) continue;
        const chg = Number(t.priceChangePercent);
        out[app] = { price, chg: isFinite(chg) ? chg : undefined };
      }
    } catch (e) {
      /* ignore this chunk; keep whatever else succeeded */
    }
  }
  return out;
}
