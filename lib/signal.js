...
export function tradePlan(asset, { edge, q, lv, ruin, nu }) {
  ...
  } else if (score >= checklist.length) {
    // Only a 100% read \u2014 every confluence check passing \u2014 is presented as an
    // actionable entry. Anything less is a caution/wait, never a clean "go".
    tone = "go";
    action = "TAKE THE ENTRY \u2014 full confluence";
    headline =
      "Every check passes \u2014 direction, location, liquidity, risk:reward and a positive edge all line up. This is an A+ setup: enter on confirmation and manage the stop.";
  } else if (score === checklist.length - 1) {
    tone = "caution";
    action = "TAKE WITH CAUTION \u2014 reduce size";
    headline =
      "Almost everything agrees, but not all of it. Only a 100% read is a clean entry \u2014 if you take this, size down and demand confirmation at the trigger.";
  } else if (score >= checklist.length - 3) {
    tone = "wait";
    action = "WAIT \u2014 needs more confluence";
    headline =
      "The setup is incomplete. Wait for a full-confluence read \u2014 let price come into the zone and confirm before committing.";
  } else {
    tone = "stop";
